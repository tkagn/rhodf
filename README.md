...
The RHODF Operator will install its components only on nodes labeled for RHODF with `cluster.ocs.openshift.io/openshift-storage=''`.
...
> Note: The dedicated/tainted nodes will only run RHODF components. The nodes will not run any apps. Therefore, if you taint, you need to have additional worker nodes that are untainted. If you don't, you will be unable to run any other apps in you OpenShift cluster.
...
Use to by deploy manifests togethere
